...
import { LifeGoal, Project, TimeBlock, Task, LLMConversation, UserPreferences } from '@/types';
...
export const useAppStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      updateLifeGoal: (id, updates) => set((state) => ({
        lifeGoals: state.lifeGoals.map(goal => goal.id === id ? { ...goal, ...updates } : goal)
      })),
      ...
      updateTimeBlock: (id, updates) => set((state) => ({
        timeBlocks: state.timeBlocks.map(block => block.id === id ? { ...block, ...updates } : block)
      })),
